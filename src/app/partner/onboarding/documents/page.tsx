...
import { ArrowLeft, UploadCloud, CheckCircle2, ShieldCheck, CircleDashed, ArrowRight } from 'lucide-react'
...
function page() {
  ...
  // tracks which docs were already submitted (from backend)
  ...
  const isUpdate = userData?.partnerStep >= 3  // already completed docs before
  ...
  // fetch existing docs if already submitted
  useEffect(() => {
    if (userData?.partnerStep >= 3) {
      axios.get("/api/auth/partner/onboarding/documents")
        .then(({ data }) => {
          if (data?.aadharUrl || data?.licenseUrl || data?.rcUrl) {
            setExisting({
        ...
        .catch(() => {}) // silently ignore — existing stays null
    }
  }, [userData])
  ...
  // allow submit if:
  // - first time: all 3 new files selected
  // - update: at least 1 new file selected (rest keep existing)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(
        "/api/auth/partner/onboarding/documents",
        formData
      )
      ...
      >
            ...
            {[0, 1, 2].map((i) => (
              <div
                ...
                className={`h-1.5 rounded-full transition-all ${
                  i <= 1 ? 'w-8 bg-black' : 'w-8 bg-gray-200'
                }`}
              />
            ))}
          </div>
        ...
        <div className='mt-8 space-y-4'>
          {DOCUMENTS.map((doc) => {
            ...
            // show as "uploaded" if a new file chosen OR already had one
            const isUploaded = !!newFile || hasExisting

            return (
              <div key={doc.id} className='rounded-2xl border border-gray-200 p-4 sm:p-5'>
                <div className='flex items-start justify-between gap-4'>
                  <div>
                    <p className='text-[18px] font-bold text-black'>{doc.title}</p>
                ...
                <button
                  ...
                  className={`group mt-3 w-full flex items-center justify-center gap-2 rounded-xl border border-dashed px-4 py-4 text-sm font-medium transition-colors duration-200 ${
                    newFile
                      ? 'border-black bg-black text-white'        // new file selected
                      : hasExisting
                      ? 'border-gray-300 text-gray-500 hover:border-black hover:bg-black hover:text-white'  // existing — re-upload
                      : 'border-gray-300 text-gray-500 hover:border-black hover:bg-black hover:text-white'  // empty
                  }`}
                >
                  <UploadCloud size={18} />
