...
// ---- Frontend-only types (populated shape, NOT the Mongoose document) ----
type BookingStatus =
  | 'requested'
  | 'awaiting_payment'
  | 'confirmed'
...
type PaymentStatus = 'pending' | 'paid' | 'cash_pending' | 'failed'
...
function BookingCard({ booking, index }: { booking: IBookingPopulated; index: number }) {
  ...
  const driverAssigned = Boolean(booking.driver?.name)
  ...
  return (
    ...
    >
      {/* Header row — driver name + phone, background tinted by booking status */}
      <div className={`flex items-center justify-between gap-3 px-4 sm:px-6 py-4 border-b border-zinc-100 ${statusCfg.bg}`}>
        <div className='flex items-center gap-3 min-w-0'>
          <div className='w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-white/70 flex items-center justify-center flex-shrink-0'>
            <UserIcon size={18} className={statusCfg.text} />
          </div>
          <div className='min-w-0'>
            <p className='font-black text-zinc-900 text-sm sm:text-base leading-tight truncate'>
              {driverAssigned ? booking.driver!.name : 'Finding driver...'}
            </p>
            <div className='flex items-center gap-1 text-zinc-500 text-xs font-medium mt-0.5'>
              <Phone size={11} />
              <span className='truncate'>
                {driverAssigned ? booking.driverMobileNumber || '—' : 'Not assigned yet'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className='bg-black width-full h-[1px]'/>
      {/* Vehicle */}
      ...
      {/* Date + Fare */}
      <div className='px-4 sm:px-6 py-3 bg-zinc-50 flex items-center justify-between border-b border-zinc-100'>
...
function Page() {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        setLoading(true)
        const { data } = await axios.get('/api/auth/booking')
    ...
    <div className='min-h-screen bg-zinc-100 px-4 sm:px-6 py-8 sm:py-12'>
      <div className='max-w-3xl mx-auto'>
        ...
        >
          ...
          <div>
            <h1 className='text-2xl sm:text-3xl font-black tracking-tight text-zinc-900'>My Bookings</h1>
